...
export default function AdminUserManagement() {
  ...
  const getServicePointName = (id) => {
    if (!id || id === "-") return "-";
    const found = servicePoints.find(
      (sp) => String(sp.service_point_id) === String(id),
    );
      ...
      .then((data) => {
        ...
        setUsers(
          arr.map((u) => ({
            ...
            position: u.position,
            role: u.user_role,
            branch: u.branch_name ?? u.branch_id,
            branchId: u.branch_id,
            servicePoint: u.service_point_id ?? "-",
            username: u.emp_code,
            canRequest: u.can_request ?? false,
            phone: u.phone,
            email: u.email,
  ...
  useEffect(() => {
    ...
    fetch(`${API}/branches`, { headers: hdr() })
      ...
      .catch(() => {});
    fetch(`${API}/service-point`, { headers: hdr() })
      .then((r) => r.json())
      .then((d) => setServicePoints(Array.isArray(d) ? d : (d.data ?? [])))
      .catch(() => {});
  }, []);

  const filtered = users.filter((u) => {
    const matchTab =
      activeTab === "ใช้งานอยู่"
        ? u.isActive
        : activeTab === "ถูกลบแล้ว"
          ? !u.isActive
          : true;
  ...
  const handleRestore = (username) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          const r = await fetch(`${API}/admin/users/${username}/restore`, {
            method: "PATCH",
            headers: hdr(),
          });
          ...
          fetchUsers();
          setOpenMenuId(null);
          showSuccess("กู้คืนผู้ใช้สำเร็จ");
        } catch {
          showError("เกิดข้อผิดพลาด กรุณาลองใหม่");
        }
      },
    ...
    try {
      const r = await fetch(`${API}/admin/users/${selectedUser.username}`, {
        ...
        body: JSON.stringify({
          ...
          branch_id: selectedUser.branchId,
          service_point_id:
            selectedUser.servicePoint === "-"
              ? null
              : Number(selectedUser.servicePoint),
  ...
  const handleAddUser = async () => {
    try {
      ...
      [
        ...
        "user_role",
        "can_request",
      ].forEach((k) => fd.append(k, newUser[k]));
      if (newUser.service_point_id)
        fd.append("service_point_id", newUser.service_point_id);
  ...
  const handleResetPassword = (username) => {
    setResetModal({ open: true, username, newPassword: "" });
    setOpenMenuId(null);
  };
  ...
  return (
    <div
      className="adminuser-root min-h-screen"
    ...
    >
      ...
      >
        <div className="px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2.5">
            <div
              className="search-focus flex items-center gap-2 rounded-xl px-3.5 py-2.5 bg-white"
          ...
          {["ใช้งานอยู่", "ถูกลบแล้ว", "ทั้งหมด"].map((t) => (
            ...
            >
              ...
              <span
                ...
                style={{
                  background:
                    activeTab === t ? "rgba(255,255,255,.25)" : "#e8f5e9",
                  color: activeTab === t ? "#fff" : "#2e7d32",
                }}
              >
                {tabCounts[t]}
              </span>
              ...
              <thead>
                ...
                >
                  {[
                    ...
                    <th
                      key={h}
                      className="py-3 px-4 text-xs font-bold text-center"
                      style={{ color: "#2e7d32" }}
                    >
                      {h}
                    </th>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="py-20 text-center">
                      <FaUsers
                        size={32}
                        style={{ color: "#c8e6c9", margin: "0 auto 8px" }}
                      />
                  ...
                  filtered.map((user) => {
                    ...
                    return (
                      <tr
                        key={user.id}
                        className={`user-row border-t ${!user.isActive && activeTab !== "ถูกลบแล้ว" ? "opacity-50" : ""}`}
                      ...
                      >
                        ...
                        <td
                          ...
                          style={{ color: "#6b7280" }}
                        >
                          {getServicePointName(user.servicePoint)}
                        </td>
                        <td
                          className="py-3.5 px-4 text-center text-xs"
                          style={{ color: "#6b7280" }}
                        ...
                        <td className="py-3.5 px-4 text-center relative">
                          ...
                          {openMenuId === user.id && (
                            <div
                              className="absolute right-10 top-12 bg-white rounded-2xl p-1.5 z-50 min-w-[170px]"
                              style={{
      ...
      {isDetailOpen && selectedUser && (
        ...
        >
          <div
            className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl modal-enter"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            >
              ...
              >
                {selectedUser.profileImage ? (
                  <img
                    src={`${API}${selectedUser.profileImage}`}
                    alt="profile"
            ...
            <div className="px-6 py-5">
              {[
                ...
                {
                  label: "จุดบริการ",
                  value: getServicePointName(selectedUser.servicePoint),
                },
                {
                  label: "สถานะ",
                  value: selectedUser.isActive ? "ใช้งานอยู่" : "ถูกลบแล้ว",
                },
                {
                  label: "สิทธิ์เบิก",
                  value: selectedUser.canRequest ? "เบิกได้" : "เบิกไม่ได้",
                },
              ].map(({ label, value }) => (
                <div
                  key={label}
      ...
      {isEditOpen && selectedUser && (
        ...
        >
          <div
            className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl modal-enter max-h-[90vh] overflow-y-auto no-scrollbar"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-center pt-3 sm:hidden">
              <div
            ...
            <div className="px-6 py-5 space-y-4">
                ...
                <Field label="จุดบริการ">
                  ...
                  >
                    ...
                    {servicePoints.map((sp) => (
                      <option
                        key={sp.service_point_id}
                        value={sp.service_point_id}
                      >
              ...
              <Field label="สิทธิ์การเบิกวัสดุ">
                ...
                >
                  ...
                  <Toggle
                    on={selectedUser.canRequest}
                    onToggle={() =>
                      setSelectedUser({
                        ...selectedUser,
                        canRequest: !selectedUser.canRequest,
                      })
                    }
                  />
                </div>
            ...
            <div className="px-6 py-5 space-y-3.5">
              {[
                ...
                },
                {
                  label: "ชื่อ-นามสกุล",
                  key: "full_name",
                  placeholder: "นายตัวอย่าง ใจดี",
                },
                {
                  label: "ตำแหน่งงาน",
                  key: "position",
                  placeholder: "เจ้าหน้าที่พัสดุ",
                },
              ...
              <Field label="จุดบริการ">
                <select
                  className="field-input"
                  value={newUser.service_point_id}
                  onChange={(e) =>
                    setNewUser({ ...newUser, service_point_id: e.target.value })
                  }
                >
                  <option value="">-- ไม่มี --</option>
                  {servicePoints.map((sp) => (
      ...
      {confirmDialog.isOpen && (
        ...
        >
          <div
            className="bg-white w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl modal-enter"
            onClick={(e) => e.stopPropagation()}
          >
      ...
      {resetModal.open && (
        <div
          ...
          onClick={() =>
            setResetModal({ open: false, username: "", newPassword: "" })
          }
        >
          <div
            className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 modal-enter"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="flex gap-3 mt-4">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const r = await fetch(
                      `${API}/admin/users/${resetModal.username}/reset-password`,
                      {
                        method: "PATCH",
                        headers: hdr(true),
                        body: JSON.stringify({
                    ...
                    if (!r.ok) throw new Error();
                    showSuccess(
                      `รีเซ็ตรหัสผ่านของ ${resetModal.username} เรียบร้อยแล้ว`,
                    );
                    setResetModal({
                      open: false,
                      username: "",
